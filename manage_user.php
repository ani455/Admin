      ...
      <div class="main-panel">
		...
		<footer class="footer">
			<div class="d-sm-flex justify-content-center justify-content-sm-between">
				<span class="text-muted d-block text-center text-sm-left d-sm-inline-block">Copyright © Jeetopkr 2025</span>
			</div>
		</footer>
      </div>     
    </div>
  ...
  <script>
	$(function () {
	 ...
	 $('#example1 thead th').each(function () {
        var title = $(this).text();
        if (title === 'Mobile' || title === 'Cust ID' || title === 'IP Address' || title === 'Refer Code' || title === 'Account No.') {
    ...
    $.ajax({
        url: 'toggle_freeze.php', // PHP file to handle update
        type: 'POST',
        data: { id: id, status: status },
        success: function(response) {
            if(response == "success") {
                location.reload(); // Reload DataTable after change
            } else {
